...
// Get all shops with tenant information
try {
    $db = Database::getInstance();
    $stmt = $db->query(
        "SELECT s.*, t.name as tenant_name, t.email as tenant_email, t.id as tenant_id 
         FROM shops s 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Check if rent record already exists for this shop and month
            $check_stmt = $db->query(
                "SELECT id FROM rents WHERE shop_id = ? AND rent_year = ? AND rent_month = ?",
                [$shop_id, $rent_year, $rent_month]
            );
            ...
            // Insert rent record
            $result = $db->query(
                "INSERT INTO rents (shop_id, rent_year, rent_month, calculated_rent, penalty, amount_waved_off, final_rent, remarks) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                [$shop_id, $rent_year, $rent_month, $calculated_rent, $penalty, $amount_waved_off, $final_rent, $remarks]
            );
            ...
            if ($result) {
                ...
                Logger::info("New rent record created for shop ID: " . $shop_id);
                $_SESSION['success'] = "Rent record created successfully";
            ...
            Logger::error("Error creating rent record: " . $e->getMessage());
            $error = "Error creating rent record: " . $e->getMessage();
        }
    } else {
...
<head>
    ...
    <title>Add New Rent - Shop Rent Management System</title>
            ...
            <div class="col-md-8 offset-md-2">
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title">Add New Rent</h2>
                    </div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <form method="POST" action="" id="rentForm">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="month" class="form-control" id="rent_of_month" name="rent_of_month" 
                                       value="<?php echo $currentMonth; ?>" required onchange="calculateRent()">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control" id="calculated_rent" 
                                       name="calculated_rent" value="0" readonly>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control" id="penalty" name="penalty" 
                                       value="0" onchange="calculateFinalRent()">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control" id="amount_waved_off" 
                                       name="amount_waved_off" value="0" onchange="calculateFinalRent()">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control" id="final_rent" 
                                       name="final_rent" value="0" readonly>
                            </div>
